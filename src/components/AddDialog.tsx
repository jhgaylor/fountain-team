...
/**
 * The add-teammate form: agent, optional name, environment (the agent's own
 * by default), optional vault. Environment and vault choices are narrowed by
 * the agent's allowlists the way the server will enforce them; the server is
 * still the judge (a hand-built request gets a 422).
 */
export function AddDialog({ client, onTeam, onAdded, onClose }: Props) {
  ...
  // A pick the newly chosen agent does not offer falls back to the default.
  useEffect(() => {
  ...
  return (
    <div className="modal-root">
      ...
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="add-title">
        <header>
          <h2 id="add-title">Add a teammate</h2>
          <button className="icon" onClick={onClose} aria-label="Close">
            ×
          </button>
        </header>
        <p className="muted small">Each teammate gets its own computer and one ongoing conversation with you.</p>
          ...
          <form onSubmit={submit} className="stack">
            ...
            <label>
              ...
              <input
                ...
                value={name}
                maxLength={120}
                placeholder={agent?.name ?? "Teammate"}
